...
function ScmPanel(props) {
    ...
    }, (0, react_1.createElement)('option', { value: '' }, `来源：HEAD（${status?.branch ?? '当前'}${status?.detached === true ? '' : ''}）`), ...branches.filter(branch => !branch.remote && !branch.current).map(branch => (0, react_1.createElement)('option', { key: `lb:${branch.name}`, value: branch.name }, branch.name)), ...branches.filter(branch => branch.remote).map(branch => (0, react_1.createElement)('option', { key: `rb:${branch.name}`, value: branch.name }, branch.name)))), ...branches.map(branch => (0, react_1.createElement)('div', { key: `${branch.remote ? 'r' : 'l'}:${branch.name}`, style: S.row, title: branch.subject }, (0, react_1.createElement)('span', { style: { ...S.badge, color: branch.current ? TOKEN.ok : TOKEN.dim } }, branch.current ? '●' : branch.remote ? '☁' : '○'), (0, react_1.createElement)('span', { style: S.path }, branch.name), (0, react_1.createElement)('span', { style: { ...S.dim, fontSize: 11 } }, branch.shortHash), branch.current || branch.remote
        ? null
        : (0, react_1.createElement)('button', {
            style: S.iconButton,
            title: `切换到 ${branch.name}`,
            onClick: () => void run(`切换 ${branch.name}`, async () => {
                await (0, api_js_1.call)('switch', sessionId, { repo: repoRef.current, branch: branch.name });
                return `已切换到 ${branch.name}`;
            }),
        }, '切换'), branch.current || branch.remote
        ? null
        : (0, react_1.createElement)('button', {
            style: S.iconButton,
    ...
    const syncBar = (0, react_1.createElement)('div', { style: { display: 'flex', gap: 4, padding: '4px 8px', borderBottom: `1px solid ${TOKEN.border}`, flexWrap: 'wrap' } }, context.remotes.length > 1
        ...
        : null, (0, react_1.createElement)('button', { style: S.button, onClick: () => void run('抓取', async () => {
            ...
            return result.output === '' ? '已是最新' : result.output;
        }) }, '拉取(合并)'), (0, react_1.createElement)('button', { style: S.button, onClick: () => {
            if (!window.confirm('推送当前分支到远程？'))
                return;
            void run('推送', async () => {
                const result = await (0, api_js_1.call)('push', sessionId, { repo: repoRef.current, remote, confirm: true });
